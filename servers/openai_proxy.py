...
# 在导入 torch 之前设置 CUDA_VISIBLE_DEVICES
# 注意：由于两个模型可能使用不同 GPU，这里设置所有可能用到的设备
if USE_EMBEDDING_GPU and USE_RERANKER_GPU:
    if EMBEDDING_CUDA_DEVICE == RERANKER_CUDA_DEVICE:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(EMBEDDING_CUDA_DEVICE)
    else:
        # 两个模型使用不同 GPU，设置两个都可见
        os.environ["CUDA_VISIBLE_DEVICES"] = f"{EMBEDDING_CUDA_DEVICE},{RERANKER_CUDA_DEVICE}"
elif USE_EMBEDDING_GPU:
    os.environ["CUDA_VISIBLE_DEVICES"] = str(EMBEDDING_CUDA_DEVICE)
elif USE_RERANKER_GPU:
    os.environ["CUDA_VISIBLE_DEVICES"] = str(RERANKER_CUDA_DEVICE)
else:
    os.environ["CUDA_VISIBLE_DEVICES"] = ""

print(f"[GPU Config] USE_EMBEDDING_GPU={USE_EMBEDDING_GPU}, EMBEDDING_CUDA_DEVICE={EMBEDDING_CUDA_DEVICE}, USE_RERANKER_GPU={USE_RERANKER_GPU}, RERANKER_CUDA_DEVICE={RERANKER_CUDA_DEVICE}, CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES')}, MODEL_DTYPE={MODEL_DTYPE_STR}")
